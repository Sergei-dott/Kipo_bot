...
Не упусти шанс заглянуть за кулисы и почувствовать дух нашего учебного заведения! Нажми на видео и начни своё приключение *прямо сейчас!*
    """
            ...
            keyboard.add(InlineKeyboardButton("Смотреть видео", url="https://youtube.com/ваше_видео"))
...
Зарабатывайте виртуальные монеты для наибольшего шанса поступить в наше учебное заведение.
    ...
    def _show_more_info(self, call: CallbackQuery) -> None:
        ...
        chat_id = call.message.chat.id
        info_text = """
В этом боте вы сможете зарабатывать монеты для увеличения среднего балла аттестата, для поступления в наше учебное учреждение.

*Как это работает ?*

Ваша задача это как можно больше набрать монет в нашем Web приложении, чем больше монет тем больше баллов вы сможете суммировать к вашему среднему баллу аттестата.
*Максимальное получение баллов составляет 0,3*.

Примерное количество монет на балансе в Web приложении для получение 0,3 балла к аттестату должно составлять 
~ *100 000 монет* и больше.

Но так же можно  получить более низкий балл набрав при этом 
~ *50 000 монет* и больше, что в итоге вы можете получить +0,2 балла к вашему аттестату. 

Если вы набрали меньше чем 
*50 000, но больше чем 30 000 монет*, то вы можете прибавить к вашему аттестату 0,1 балл.
В другом случае если баланс составляет менее *30 000 монет*, то баллы будут не начислены 😔
        ...
        keyboard.row(
            ...
            InlineKeyboardButton("Подключить крипто кошелек", callback_data='connect_wallet')
        )
    ...
    def _show_convert_coins(self, chat_id: int) -> None:
        ...
        text = """
*Как конвертировать монеты в реальные баллы ?*
...
Далее в течение недели после окончания игры, на ваш крипто кошелёк будет начислено N количество криптовалюты *KIPO*, это сумма и будет прибавлена к вашей средней оценке аттестат. 
Чтобы подтвердить начисленные вам монеты *KIPO*, во время начала подачи документов в наше учебное заведение, вы будете должны показать приемной комиссии сумму ваших набранных монет в самом крипто кошелке.
 Если вы прошли все этапы успешно, то мы вас поздравляем, вы прибавили N количество баллов к  _вашей средней оценки аттестата_. 😀
        """
        ...
        try:
            sent_msg = self.bot.send_photo(
                chat_id=chat_id,
                photo=open(os.path.join('image', 'KIPOKOSH1.jpg'), 'rb'),
                caption=text,
                reply_markup=keyboard,
                parse_mode='Markdown'
            )
...
Если сделали все правильно, то  *мы вас поздравляем* вы  успешно подключили свой кошелек к нашему приложению, что вскоре вы сможете получить _свое вознаграждение_ 😀
        ...
        keyboard.row(
            ...
            InlineKeyboardButton("Пошаговое видео", url="https://rutube.ru/shorts/44930c106504842c9e0cc6ad03858df2?r=wd")
        )
        keyboard.row(
...
if __name__ == '__main__':
    TOKEN = '7676539441:AAEfMg0eQjokW4huz0h70G5CCfbzQYsF1fI'
    bot = KIPOBot(TOKEN)
    bot.run()
